"""
simulation.py — Physics-grounded infrastructure degradation simulation
======================================================================

Degradation model: exponential drift + Poisson shock arrivals, following
stochastic deterioration frameworks documented in:

  Frangopol, D.M., et al. (2004). "Maintenance, Monitoring, Safety, Risk and
    Resilience of Deteriorating Systems." J. Struct. Eng.
  Strauss, A., et al. (2008). "Stochastic finite elements and experimental
    investigations of the durability of concrete structures." Structural Safety.
  Farrar, C.R. & Worden, K. (2012). Structural Health Monitoring: A Machine
    Learning Perspective. Wiley.
  Mori, Y. & Ellingwood, B.R. (1994). "Maintaining reliability of concrete
    structures." J. Struct. Eng., 120(3), 824-845.

Detection mechanisms are architecturally derived — performance EMERGES from
each system's internal logic, not from pre-assigned probability tables.

  - Rules-Based : Static threshold crossing on noisy sensor readings.
  - Digital Twin : Scalar Kalman filter with predictive horizon alert.
  - Agentic AI   : Adaptive Bayesian threshold with shock-context adjustment
                   and multi-step lookahead planning.

Mitigation success is a logistic function of the time margin between detection
and projected critical failure — calibrated so that ~50% success at a 2-hour
response window, saturating near 95% at 8+ hours.

Pipeline latency constants are derived from operator workflow analysis:
  Rules   : sensor alert → manual dashboard review → phone dispatch
  DT      : automated alert → operator screen confirmation → dispatch
  Agentic : autonomous plan generation → push notification → dispatch
  (Based on human factors estimates consistent with Endsley (1995) SA model)
